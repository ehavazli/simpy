...
def main(argv):
    ...
    *******************************************
    ...
    for n in ts_list:
        ...
        year = int(year)
        if year in rmse_avg: pass
        else:
            print 'Working on '+str(year)+' year long time series'
            spl_lst = glob.glob(directory+'/syn'+str(year)+'*')
            data_to_plot = []
            rmse_sum = zeros((1000,1000))
            for i in spl_lst:
                velocity_file = i +'/velocity_simNRmse.h5'
                f = h5py.File(velocity_file,'r')
                dset = f['velocity'].get('velocity')
                data_to_plot.extend(dset)
                rmse_sum += array(dset)
            avg = rmse_sum/len(spl_lst)
            rmse_avg[year] = avg

            h5file = 'average_NRmse_'+str(year)+'_years.h5'
            g = h5py.File(h5file,'w')
            hh = g.create_group('velocity')
            dset = hh.create_dataset('velocity', data=avg, compression='gzip')

            for key, value in f['velocity'].attrs.iteritems():
                hh.attrs[key] = value
            g.close()

    sorted_nrmse = collections.OrderedDict(sorted(rmse_avg.items()))
    nrmse_values = []

    for key, values in sorted_nrmse.iteritems():
        nrmse_values.append(values*1000.0) #Convert from meters to milimeters
        labels.append(key)
...
# Save the figure
    fig.savefig('box_plot_VelNRmse.tiff', bbox_inches='tight', dpi = 300)
    plt.close()
