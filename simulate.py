...
def main(argv):
    ...
    *******************************************
       ...
       signals to include: strato, turbulent, deformation, atm_all, all
...
#    title=fig.suptitle('STRATO')
#    im = ax.imshow(strato,cmap='jet',origin='lower')
#    cb = fig.colorbar(im)
#    #cb.set_label('meters')
...
#    fig = plt.figure()
#    ax = fig.add_axes([0.1,0.1,0.8,0.8])
#    title=fig.suptitle('DEM')
#    im = ax.imshow(dem_tif,cmap='jet',origin='lower')
#    cb = fig.colorbar(im)
#    cb.set_label('meters')
#    fig.savefig('dem.png')
#    plt.close()
...
############################################################################################################
    ...
    #   par = [rx,ry,z,radius,pressure change,modulus(shear or youngs)]
        ...
        if n <= n_im: 
            n += 1
    #        atmo_strat = scipy.io.loadmat('./atm_all/atmo_strat.mat')
    #        atmo_turb = scipy.io.loadmat('./atm_all/atmo_surf_turb_'+str(n)+'.mat')
    #        atmo_noise_turb = atmo_turb['fsurf']*0.01
    #        random_strat = random.uniform(-0.0075,0.0075)
    #        random_strat = random.uniform((0),(4*pi))    
    #        atmo_noise_strat = random_strat * strato
    #        atmo_noise_strat = strato

    
    #        atmo_noise_strat = atmo_strat['fsurf'] * random.uniform(0,4)        
    #        atmo_noise_turb = atmo_noise_turb*range2phase #Convert from mm to phase
    #        atmo_noise_strat = atmo_noise_strat*range2phase #Convert from mm to phase
    ...
    #cb.set_label('meters')
               ...
               image = wrap(rand_phase+(atmo_noise_strat))

            elif inc_signal == 'turbulent':
                random_turb = int(random.uniform(1,110))
                atmo_turb = scipy.io.loadmat('./atm_all/atmo_surf_turb_'+str(random_turb)+'.mat')
                random_turb = random.uniform(0,1) 
                atmo_noise_turb = atmo_turb['fsurf'] * 0.1 * range2phase * random_turb
                image = wrap(rand_phase+atmo_noise_turb)

            elif inc_signal == 'deformation':
#                image = wrap(rand_phase+deformation)
                image = wrap(deformation)

            elif inc_signal == 'atm_all':
                ds = gdal.Open("socorro_test.tif")
                dem_tif = array(ds.GetRasterBand(1).ReadAsArray())
                dem_inv = (dem_tif)
                norm_max = float(amax(dem_inv))
...
#                image = wrap(rand_phase+atmo_noise_strat+atmo_noise_turb)
                image = wrap(atmo_noise_strat+atmo_noise_turb)
...
#                fig = plt.figure()
#                ax = fig.add_axes([0.1,0.1,0.8,0.8])
#                title=fig.suptitle(str(date_2)+" Psuedo-Random Phase")
#                im = ax.imshow(rand_phase,cmap='jet',origin='lower')
#                cb = fig.colorbar(im)
#                cb.set_label('Phase[radians]')
#                fig.savefig(directory+date_2+'.rand_phase.png')
#                plt.clf()
######################
#                fig = plt.figure()
#                ax = fig.add_axes([0.1,0.1,0.8,0.8])
#                title=fig.suptitle(str(date_2)+" Deformation")
#                im = ax.imshow(deformation,cmap='jet',origin='lower')
#                cb = fig.colorbar(im)
#                cb.set_label('Phase[radians]')
...
#####################
    ...
    #         im = ax.imshow(atmo_noise_turb,cmap='jet',origin='lower')
    #         cb = fig.colorbar(im)
    #         cb.set_label('cm')
...
######################
                ...
                fig = plt.figure()
                ax = fig.add_axes([0.1,0.1,0.8,0.8])
                title=fig.suptitle(str(date_2)+" Stratified Atmospheric Noise: "+ str(round(random_strat,2)))
                im = ax.imshow(atmo_noise_strat,cmap='jet',origin='lower')
    #        im = ax.imshow(atmo_noise_strat,cmap='jet',origin='lower')
                cb = fig.colorbar(im)
                cb.set_label('Phase [radians]')
                ...
                fig = plt.figure()
                ax = fig.add_axes([0.1,0.1,0.8,0.8])
                title=fig.suptitle(str(date_2)+" Turbulent Atmospheric Noise: "+str(round(random_turb,2)))
                im = ax.imshow(atmo_noise_turb,cmap='jet',origin='lower')
                cb = fig.colorbar(im)
                cb.set_label('Phase [radians]')
                fig.savefig(directory+date_2+'.atmo_noise_turb.png')
